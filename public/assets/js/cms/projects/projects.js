...
const dataTableLanguage = currentLanguage === "ar"
    ...
    : '';
var KTAppCmsProjects = function () {
    return {
        init: function () {
            const table = $("#kt_cms_projects_table").DataTable({
                processing: true,
                serverSide: true,
                language: { url: dataTableLanguage },
                ajax: {
                    url: '/' + lang + '/cms/admin/projects/',
                    type: "GET",
                },
                columns: [
            ...
            table.on("click", '[data-kt-cms-project-filter="delete_row"]', function () {
                const projectId = $(this).closest("div[data-project-id]").data("project-id");
                const projectName = $(this).closest("div[data-project-id]").data("project-name");

                Swal.fire({
                    text: `${messages.Deleted} ${projectName}?`,
                    icon: "warning",
                    ...
                    if (result.value) {
                        Swal.fire({
                            text: messages.Deleting + projectName + "...",
                            icon: "info",
                            showConfirmButton: false,
                            allowOutsideClick: false
                        });
                        $.ajax({
                            url:  '/' + lang + '/cms/admin/projects/' + projectId,
                            type: "DELETE",
                        ...
                        Swal.fire({
                            text: projectName + messages.NotDeleted,
                            icon: "error",
                            buttonsStyling: false,
                            confirmButtonText: messages.confirmButtonText,
                            customClass: { confirmButton: "btn fw-bold btn-primary" }
