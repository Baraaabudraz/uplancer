...
@section('title')
    {{trans('dashboard_trans.Edit post')}}
@endsection
@section('content')
    <!--begin::Toolbar-->
    <div class="toolbar" id="kt_toolbar">
        ...
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            ...
            <div data-kt-place="true" data-kt-place-mode="prepend"
                ...
                <!--begin::Title-->
                <h1 class="d-flex align-items-center text-dark fw-bolder my-1 fs-3">{{trans('dashboard_trans.Edit post')}}</h1>
                <!--end::Title-->
                <!--begin::Separator-->
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                    ...
                    <!--end::Item-->
                    <!--begin::Item-->
                    <li class="breadcrumb-item text-dark">{{trans('dashboard_trans.Edit post')}}</li>
                    <!--end::Item-->
                </ul>
                <!--end::Breadcrumb-->
            </div>
            ...
            <div class="card card-xl-stretch mb-5 mb-xl-8">
                ...
                <div class="card-body pt-0">
                    ...
                    <div class="mb-13 text-center">
                        <!--begin::Title-->
                        <h1 class="mb-3">{{trans('dashboard_trans.Edit post')}}</h1>
                        <!--end::Title-->
                        <!--begin::Description-->
                        <div class="text-gray-400 fw-bold fs-5">{{trans('dashboard_trans.You can browse the list of posts')}}
                            <a href="{{ route('posts.index') }}" class="fw-bolder link-primary">{{trans('dashboard_trans.here')}}</a>.
                        </div>
                        <!--end::Description-->
                    </div>
                    <!--end::Heading-->
                    <form action="{{route('posts.update',$posts->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between mb-5">
                            <h4 class="m-0 font-weight-bold text-primary">{{trans('dashboard_trans.Edit post')}}</h4>
                        </div>
                        <div class="row">
                            <div class="col-md-12 mb-10" style="border:1px ">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 d-flex flex-column mb-8 fv-row">
                                        ...
                                        <input type="text" name="name[{{$key}}]" value="{{$posts->getTranslation('name',$key)}}" class="form-control @error('name') is-invalid @enderror "
                                               id="" aria-describedby="" placeholder="Enter name of post">
                                        @error('name.',$key)
                                    ...
                                    @endforeach
                                    <div class="col-md-6 d-flex flex-column mb-8 fv-row">
                                        <label class="d-flex align-items-center fs-6 fw-bold mb-2">{{trans('dashboard_trans.Sections')}}</label>
                                        <select class="form-select" name="section_id" id="section_id">
                                            <option disabled hidden selected>{{trans('dashboard_trans.All sections')}}</option>
                                            @foreach($sections as $section)
                                                <option value="{{$section->id}}" @if($posts->section_id == $section->id) selected @endif>{{$section->name}}</option>
                                            @endforeach
                                        </select>
                                        @error('section_id')
                                        <span class="text-danger" role="alert">{{ $message }}</span>
                                        ...
                                        @foreach(config('lang') as $key=>$lang)
                                    <div class="col-md-6 d-flex flex-column mb-8 fv-row">
                                        <label class="d-flex align-items-center fs-6 fw-bold mb-2">{{trans('dashboard_trans.Post')}} ({{$lang}})</label>
                                        <textarea class="form-control" name="post[{{$key}}]" id="post">{{$posts->getTranslation('post',$key)}}</textarea>
                                        @error('post.',$key)
                                        <span class="text-danger" role="alert">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="image-input image-input-outline"  data-kt-image-input="true"
                                            ...
                                            <div class="image-input-wrapper w-400px h-300px"
                                                 style="background-image: url({{url('images/post/',$posts->image)}});object-fit: cover"></div>
